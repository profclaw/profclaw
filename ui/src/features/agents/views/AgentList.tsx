import { Bot, Cpu, Sparkles, Activity, Clock, Settings, ShieldCheck, Zap, Brain, Layers } from 'lucide-react';
...
export function AgentList() {
  ...
  const { data: skillsOverview } = useQuery({
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-2">
        {/* Smart Routing Stats */}
        <Card className="p-4 bg-gradient-to-br from-emerald-50/50 to-teal-50/50 dark:from-emerald-950/20 dark:to-teal-950/20">
          ...
          {routing?.data ? (
            <div className="space-y-2">
              <div className="text-2xl font-bold text-emerald-700 dark:text-emerald-300">
                {routing.data.totalSaved}
              </div>
              <div className="text-xs text-emerald-600 dark:text-emerald-400">
                saved across {routing.data.totalRouted} queries ({routing.data.avgSavingsPercent} avg)
              </div>
              <div className="flex gap-2 mt-2">
                {Object.entries(routing.data.byTier).map(([tier, count]) => (
                  <span key={tier} className="px-2 py-0.5 rounded-full bg-emerald-100 dark:bg-emerald-900/30 text-xs">
            ...
            <p className="text-xs text-muted-foreground">Routing stats will appear as you use profClaw</p>
          )}
        </Card>

        {/* Learning Progress */}
        <Card className="p-4 bg-gradient-to-br from-violet-50/50 to-indigo-50/50 dark:from-violet-950/20 dark:to-indigo-950/20">
          <div className="flex items-center gap-2 mb-3">
            <Brain className="h-4 w-4 text-violet-600" />
            <h4 className="font-medium text-sm">Learning</h4>
          </div>
          {insights?.data ? (
            <div className="space-y-2">
              <div className="text-2xl font-bold text-violet-700 dark:text-violet-300">
                {insights.data.stats.total}
              </div>
              <div className="text-xs text-violet-600 dark:text-violet-400">
                experiences learned ({insights.data.learningRate})
              </div>
              {insights.data.topToolChains.length > 0 && (
                <div className="text-xs text-muted-foreground mt-2">
                  Top chain: {insights.data.topToolChains[0].tools.join(' -> ')}
                </div>
              )}
            </div>
          ) : (
            <p className="text-xs text-muted-foreground">Learning data appears as profClaw works</p>
          )}
        </Card>

        {/* Skills Overview */}
        <Card className="p-4 bg-gradient-to-br from-blue-50/50 to-sky-50/50 dark:from-blue-950/20 dark:to-sky-950/20">
          <div className="flex items-center gap-2 mb-3">
          ...
          {skillsOverview?.data ? (
            <div className="space-y-2">
              <div className="text-2xl font-bold text-blue-700 dark:text-blue-300">
                {skillsOverview.data.total}
              </div>
              <div className="text-xs text-blue-600 dark:text-blue-400">
                {skillsOverview.data.enabled} active, {skillsOverview.data.mcpServers} MCP servers
              </div>
              <div className="flex gap-2 mt-2">
                <span className="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-xs">
                  {skillsOverview.data.userInvocable} user
                </span>
                <span className="px-2 py-0.5 rounded-full bg-blue-100 dark:bg-blue-900/30 text-xs">
                  {skillsOverview.data.modelInvocable} AI
                </span>
              </div>
            </div>
          ) : (
            <p className="text-xs text-muted-foreground">Skills data loading...</p>
          )}
        </Card>
      </div>
    </div>
